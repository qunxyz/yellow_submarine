...
static int emulate_cpu(struct pt_regs *regs)
{
	...
	debug("op0=%x op1=%x op2=%x op3=%x", op0, op1, op2, op3);
...
int x86_hw_breakpoint_init(void)
{
	...
	debug("dr0=%lx dr1=%lx dr2=%lx dr3=%lx dr6=%lx dr7=%lx",
		bps.old_dr[0], bps.old_dr[1], bps.old_dr[2], bps.old_dr[3], bps.old_dr6, bps.old_dr7);
 ...
 */
int x86_hw_breakpoint_register(int dr_nr, unsigned long addr, int type, int len, bp_handler handler)
{
	unsigned long dr7;
	...
	if (rk_cfg.dr_protect)
		bps.dr7 |= DR_GD;
	debug("dr%d=0x%lx dr7=%lx", dr_nr, addr, bps.dr7);
...
int x86_hw_breakpoint_unregister(int dr_nr)
{
	...
	bps.dr7 &= ~(0x2 << dr_nr * 2);
	debug("dr%d=0x%lx dr7=%lx", dr_nr, bps.dr[dr_nr], bps.dr7);
	on_each_cpu_set_dr(dr_nr, bps.dr[dr_nr]);
	on_each_cpu_set_dr(7, bps.dr7);
